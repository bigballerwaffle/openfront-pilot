...
export function validateModel(raw) {
  ...
  if (
    ...
    !bounded(d.wins, d.total) ||
    !Array.isArray(d.contexts) ||
    d.contexts.length > LIMITS.contexts ||
    !Array.isArray(d.history) ||
    d.history.length > LIMITS.history ||
    !Array.isArray(d.seen) ||
    d.seen.length > LIMITS.seen
  )
  ...
  const contexts = d.contexts.map((c) => {
    if (
      ...
      c.arms.length !== VARIANTS.length
    )
      throw new Error('Invalid strategy statistics.');
    const arms = c.arms.map((a) => {
      if (
        !Number.isInteger(a.games) ||
        !bounded(a.games) ||
        !Number.isInteger(a.wins) ||
        !bounded(a.wins, a.games) ||
        !bounded(a.weight, 51) ||
        !bounded(a.reward, a.weight + 1e-9)
      )
  ...
  const history = d.history.map((h) => {
    if (
      !text(h.id, 100) ||
      !text(h.context, 180) ||
      !VARIANTS.some((v) => v.id === h.variant) ||
      typeof h.win !== 'boolean' ||
      !bounded(h.ticks) ||
      !bounded(h.commands) ||
      !bounded(h.coverage, 1) ||
      !bounded(h.date, 1e15)
    )
    ...
    if (h.territory != null) {
      const t = h.territory;
      if (
        !bounded(t.peakShare, 1) ||
        !bounded(t.averageShare, t.peakShare + 1e-9) ||
        !bounded(t.observedTicks, h.ticks)
      )
  ...
  if (
    ...
    outcomes.length > Math.min(d.total, LIMITS.outcomes) ||
    outcomes.some((w) => typeof w !== 'boolean')
  )
...
export const optionsKey = (o) =>
  JSON.stringify([o.profile, o.economy, o.navy, o.nukes, o.learning, o.diplomacy !== false]);
...
export class LearningStore {
  ...
  select(key) {
    this.reload();
    const c = this.context(key),
      stats = c?.arms ?? VARIANTS.map(arm);
    const untried = stats.findIndex((a) => a.games === 0);
    let index = untried;
    if (index < 0) {
      const total = stats.reduce((s, a) => s + a.weight, 0);
      const score = (a) =>
        (a.reward + 1) / (a.weight + 2) + 0.35 * Math.sqrt(Math.log(total + 1) / (a.weight + 1));
  ...
  async record(summary, accept = () => true) {
    const commit = () => {
      ...
      if (
        index < 0 ||
        typeof summary.win !== 'boolean' ||
        !text(summary.id, 100) ||
        !text(summary.context, 180)
      )
  ...
  winRate(window = 20) {
    const requested = Number.isFinite(Number(window))
      ? Math.max(1, Math.min(LIMITS.outcomes, Math.floor(Number(window))))
      : 20;
    const outcomes = this.data.outcomes.slice(-requested),
