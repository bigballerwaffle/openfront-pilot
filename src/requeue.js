...
export function consumeRequeue(storage, now = Date.now()) {
  try { const raw = storage.getItem(REQUEUE_KEY); storage.removeItem(REQUEUE_KEY);
    const data = JSON.parse(raw); return data?.expires > now && data.expires <= now + 120000;
  } catch { return false; }
}
export class AutoRequeue {
  constructor(adapter, storage, finish, navigate) {
    Object.assign(this, { adapter, storage, finish, navigate }); this.busy = false; this.navigating = false; this.epoch = 0;
  }
  stop() { this.epoch++; this.navigating = false; try { this.storage.removeItem(REQUEUE_KEY); } catch {} }
  async step(active) {
    ...
    try {
      let timer;
      try { await Promise.race([this.finish(), new Promise(resolve => { timer = setTimeout(resolve, 1500); })]); }
      ...
      this.navigating = true; this.navigate('/'); return true;
