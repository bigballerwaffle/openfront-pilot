export const U = Object.freeze({
  city: 'City', port: 'Port', factory: 'Factory', defense: 'Defense Post',
  sam: 'SAM Launcher', silo: 'Missile Silo', warship: 'Warship',
  transport: 'Transport', atom: 'Atom Bomb', hydrogen: 'Hydrogen Bomb', mirv: 'MIRV'
});
// Validate each returned type even if a client ignores the requested filter.
export const unitsOf = (view, ...types) => view.units(...types).filter(u =>
  types.includes(u.type?.()) && (!u.isActive || u.isActive()));
...
export function unitData(u) {
  return { id: u.id(), type: u.type(), tile: u.tile(), level: u.level(),
    building: u.isUnderConstruction?.() ?? false, owner: u.owner().smallID(), raw: u };
}
