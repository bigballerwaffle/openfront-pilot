...
export function sampleRegions(g) {
  const step = Math.max(1, Math.ceil(Math.sqrt(g.width() * g.height() / 12000)));
  ...
  for (let y = Math.floor(step / 2); y < g.height(); y += step) {
    for (let x = Math.floor(step / 2); x < g.width(); x += step) {
      ...
      r.n++; r.x += x; r.y += y; r.moment += x * x + y * y;
      regions.set(id, r);
    }
  }
...
export function compactnessScore(s, enemy) {
  ...
  const frontage = Number.isFinite(enemy.count) && enemy.area > 0
    ? clamp(45 * (enemy.count / Math.sqrt(enemy.area) - .75), -35, 75) : 0;
