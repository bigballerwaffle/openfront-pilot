...
export function observeUpdates(game, callback) {
  ...
  function wrapped(...args) {
    ...
    try { callback(args[0]); } catch { /* Never interrupt a game update. */ }
    return result;
  }
  try { game.update = wrapped; } catch { return null; }
...
export class MatchLearning {
  constructor(store, onChange = () => {}) {
    this.store = store; this.onChange = onChange; this.run = null;
    this.seenGames = new WeakSet(); this.message = 'Learning is ready. Start a match to collect a result.';
    this.lastSave = Promise.resolve();
    this.generation = 0;
  }
  info() {
    return { ...this.store.summary(this.run?.context), message: this.message,
      variant: this.run?.variant.name ?? null, active: Boolean(this.run && !this.run.done) };
  }
  notify(message) { if (message) this.message = message; this.onChange(this.info()); }
  begin(s, options) {
    ...
    const run = { game: g, context, variant, id: `${gameID}:${clientID}`,
      identity: { clientID, team: me.team?.() ?? null },
      options: optionsKey(options), lastTick: s.tick, totalTicks: Math.max(0, g.ticksSinceStart()),
      territory: new TerritoryReward(g, me), botTicks: 0, commands: 0, wasAlive: true, running: true, done: false, excluded: '', restore: null };
    if (run.id.length > 100) { this.notify('Learning unavailable: unsupported match identifier.'); return VARIANTS[0]; }
    this.run = run; this.seenGames.add(g);
  ...
  advance() {
    ...
    if (!Number.isFinite(tick) || tick < r.lastTick) { this.invalidate('Game clock changed unexpectedly.'); return; }
  ...
  poll(game, running, options, sameGame = true) {
    ...
    this.inspect();
    if (r.done) return;
    if (!sameGame || game !== r.game) { this.finish(null, 'Match left before a result was observed.'); return; }
    if (optionsKey(options) !== r.options) this.invalidate('Settings changed during this match.');
    r.running = running;
  }
  control(running) { this.advance(); if (this.run && !this.run.done) this.run.running = running; }
  command() { if (this.run && !this.run.done) this.run.commands++; }
  invalidate(reason) {
  ...
  finish(win, reason = 'No reliable result was observed.') {
    ...
    r.done = true; r.restore?.();
    const coverage = Math.min(1, r.botTicks / Math.max(1, r.totalTicks));
    const excluded = r.excluded || (win === null ? reason : '')
      || (r.botTicks < 30 || r.commands < 1 ? 'Not enough bot play to evaluate.' : '')
      || (coverage < .8 ? 'The bot controlled less than 80% of your time alive.' : '');
    if (excluded) { this.notify(`No learning update: ${excluded}`); return this.lastSave; }
    const territory = r.territory.summary();
    const reward = matchReward(win, territory);
    const summary = { reward, territory, id: r.id, context: r.context, variant: r.variant.id, win,
      ticks: r.totalTicks, commands: r.commands, coverage };
    const generation = this.generation;
    const accept = () => this.generation === generation;
    this.lastSave = this.store.record(summary, accept).then(saved => {
      if (!accept()) return;
      this.notify(saved ? `${win ? 'Win' : 'Loss'} learned · reward ${(reward * 100).toFixed(1)}/100${territory ? ` · peak land ${(territory.peakShare * 100).toFixed(1)}%` : ' · territory unavailable'}.`
        : 'This match was already learned; it was not counted twice.');
    }).catch(() => { if (accept()) this.notify('The result could not be saved. Previous learning is preserved.'); });
  ...
  abort() {
    ...
    const r = this.run;
    if (r) {
      r.done = true; r.running = false; r.excluded = 'Emergency stop.';
      r.restore?.(); r.restore = null;
    }
    this.notify('Learning stopped. This match will not produce another learning update.');
  }
  close() { this.inspect(); return this.finish(null, 'Page closed before the result was observed.'); }
}
