...
export class PilotController {
  constructor(adapter, report = () => {}, options = {}, learning = null) {
    this.adapter = adapter; this.report = report; this.options = { ...DEFAULTS, ...options };
    this.running = false; this.epoch = 0; this.pending = null; this.busy = false;
    this.lastTick = null; this.tickTime = 0; this.commands = 0;
  ...
  start({ explicit = false } = {}) {
    ...
    if (!this.adapter.connect()) { this.report({ status: 'waiting', message: 'Join a match, choose a spawn, then press Start.' }); return; }
    const same = this.game === this.adapter.game;
    this.game = this.adapter.game; this.epoch++; this.running = true; this.stopped = false;
    this.pending = null; this.lastTick = null;
  ...
  pause(message = 'Paused. You have control.', interrupted = false) {
    ...
    this.running = false; this.epoch++; this.pending = null;
  ...
  emergencyStop(message = 'Emergency stop: bot and learning halted. Click Start to resume.') {
    ...
    this.running = false; this.stopped = true; this.epoch++; this.pending = null;
    try { this.learning?.abort(); } catch { /* Commands remain stopped even if cleanup fails. */ }
    this.report({ status: 'stopped', message });
  }
  async step(now = Date.now()) {
    if (this.stopped) return;
    try { this.learning?.poll(this.game, this.running, this.options, this.adapter.sameGame(this.game)); }
    catch { this.learning?.invalidate('Learning could not read the match state.'); }
    ...
    try {
      if (!this.adapter.sameGame(this.game)) {
        if (this.adapter.connect() && this.adapter.game !== this.game) {
          this.game = this.adapter.game; this.epoch++; this.pending = null; this.lastTick = null;
      ...
      if (s.tick !== this.lastTick) { this.lastTick = s.tick; this.tickTime = now; }
      else {
        ...
        if (now - this.tickTime > 10000) this.report({ status: 'waiting', message: 'Waiting for the game clock to resume. Pilot remains armed.' });
        return;
      }
      ...
      if (this.pending) {
        if (this.adapter.confirmed(s, this.pending)) {
          ...
          const expired = this.pending; this.pending = null;
      ...
      if (action.kind === 'wait') { this.report({ status: 'running', message: action.reason }); return; }
      const sent = await this.adapter.execute(s, action, active);
      if (!active()) return;
      if (sent) {
        this.pending = sent; this.commands++;
